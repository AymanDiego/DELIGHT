...
def parse_args():
    ...
    parser.add_argument('--model_dir', type=str, default='models/run_old_2', help='Directory to save the model checkpoints')
...
def train_conditional_flow_model(flow_model, data_train, context_train, data_val, context_val, num_epochs=1000, batch_size=512, learning_rate=1e-3, model_dir='models/', noise_magnitude=0.1, energy_threshold=5000, weight_decay=1e-5):
    ...
    df.to_csv(f"/web/aratey/public_html/delight/nf/models_nf/run_old_2_hd64/loss.csv")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.savefig(f"/web/aratey/public_html/delight/nf/models_nf/run_old_2_hd64//loss.png", bbox_inches='tight', dpi=300)
    plt.savefig(f"/web/aratey/public_html/delight/nf/models_nf/run_old_2_hd64//loss.pdf", bbox_inches='tight')
